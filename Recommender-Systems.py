...
cost = [0.1, 0.3, 1, 3, 10, 30]
error = [1.01637306285, 0.932654553545, 0.877025797715, 0.850895183488, 
         0.824033786882, 0.777168617149]
CVerror = [1.0422823422, 0.962722662811, 0.929758406982, 0.959016550869, 
           0.952583414049, 1.01403949125]
...
#error_history = np.load('error_history.npy')[:100]
...
U = pd.DataFrame(np.load('U.npy'), index=userID)
M = pd.DataFrame(np.load('M.npy'), index=movieID)
...
# Recommended movies are sorted by the release date, newest first.
